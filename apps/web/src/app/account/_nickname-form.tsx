...
export function NicknameForm({ currentName }: { currentName: string }) {
  ...
  return (
    <form action={action} className="space-y-3 bg-white border border-line rounded-card p-4">
      ...
      <div className="flex gap-2">
        <input
          ...
          maxLength={100}
          disabled={pending}
          className="flex-1 text-sm border border-line rounded-card px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent-300 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={pending}
